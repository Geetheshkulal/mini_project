...
const CplusQuiz = ({ navigation }) => {
  ...
  useEffect(() => {
    sendBotMessage('Generate a simple c++ programming question with four options.');
  }, []);

  const sendBotMessage = async (prompt, retries = 3) => {
    setIsBotTyping(true);
    try {
        ...
        if (content && content.parts && content.parts.length > 0) {
          ...
          if (splitIndex !== -1) {
            ...
            if (options.length > 0) {
              ...
              throw new Error('Invalid response format: missing question or options');
            }
          } else {
            throw new Error('Invalid response format: missing "*Answer:*" separator');
          }
        } else {
      ...
      if (retries > 0) {
        ...
        setTimeout(() => sendBotMessage(prompt, retries - 1), 1000);
      } else {
